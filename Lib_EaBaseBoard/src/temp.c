...
int32_t temp_read (void)
{
    ...
    while(GET_TEMP_STATE == state); // reads GPIO temp and checks if it changed
    state = !state;

    t1 = getTicks(); // num of milliseconds

    // Change to interrupt for better performance (rising/falling edge)
    ...
    if (t2 > t1) {
        ...
        t2 = (0xFFFFFFFF - t1 + 1) + t2; // In the case of overflow
    }
